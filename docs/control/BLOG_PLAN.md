...
| ID | Working title (EN) | Hook / angle | Source | Status |
|---|---|---|---|---|
| B1 | Scaling a trading system to zero on nights and weekends | the cost-auto-stop scheduler is the #1 lever | `scheduler_stack.py`; ADR | idea |
...
| C5 | Cascade debt: when one red pipeline hides eight layers | reactivating a long-red job surfaces debt one step at a time | ci-pipeline-cascade notes | idea |
...
| ID | Working title (EN) | Hook / angle | Source | Status |
|---|---|---|---|---|
| D1 | Why TimescaleDB hypertables for market data | vs DynamoDB, vs plain Postgres | ADR-002; DATABASE.md | idea |
